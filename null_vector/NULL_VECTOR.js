...
var ASM_CONSTS = {
  19411376: () => { if (window.__hideBootOverlay) { window.__hideBootOverlay(); } },  
 19411442: () => { try { var text = localStorage.getItem('nv_graphics_settings'); if (!text) { return 0; } var length = lengthBytesUTF8(text) + 1; var buffer = _malloc(length); stringToUTF8(text, buffer, length); return buffer; } catch (e) { return 0; } },  
 19411681: ($0) => { try { localStorage.setItem('nv_graphics_settings', UTF8ToString($0)); } catch (e) { } },  
 ...
 19417878: ($0) => { window.miniaudio.untrack_device_by_index($0); },  
 19417928: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = window.miniaudio.device_state.started; },  
 19418067: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; },  
 19418207: () => { if (typeof Module !== 'undefined' && typeof Module.isMobileWeb === 'boolean') { return Module.isMobileWeb ? 1 : 0; } var touchCapable = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0); var coarsePointer = window.matchMedia('(pointer: coarse)').matches; var noHover = window.matchMedia('(hover: none)').matches; var ua = navigator.userAgent; var mobileUa = ua ? /Android|webOS|iPhone|iPad|iPod|Mobile|Opera Mini|IEMobile/i.test(ua) : false; return (touchCapable && (coarsePointer || noHover || mobileUa)) ? 1 : 0; },  
 19418737: ($0, $1, $2, $3) => { var touches = (typeof Module !== 'undefined' && Module.activeTouches) ? Module.activeTouches : []; var count = Math.min(touches.length, $3); for (var i = 0; i < count; ++i) { var touch = touches[i]; setValue($0 + (i * 4), touch.x, 'float'); setValue($1 + (i * 4), touch.y, 'float'); setValue($2 + i, touch.zone | 0, 'i8'); } return count; },  
 19419080: () => { debugger; }
};
var wasmImports = {
  /** @export */
