...
var ASM_CONSTS = {
  38019280: () => { if (window.__hideBootOverlay) { window.__hideBootOverlay(); } },  
 38019346: () => { try { var text = localStorage.getItem('nv_controls'); if (!text) { return 0; } var length = lengthBytesUTF8(text) + 1; var buffer = _malloc(length); stringToUTF8(text, buffer, length); return buffer; } catch (e) { return 0; } },  
 38019576: ($0) => { try { localStorage.setItem('nv_controls', UTF8ToString($0)); } catch (e) { } },  
 38019657: () => { try { var text = localStorage.getItem('nv_graphics_settings'); if (!text) { return 0; } var length = lengthBytesUTF8(text) + 1; var buffer = _malloc(length); stringToUTF8(text, buffer, length); return buffer; } catch (e) { return 0; } },  
 38019896: ($0) => { try { localStorage.setItem('nv_graphics_settings', UTF8ToString($0)); } catch (e) { } },  
