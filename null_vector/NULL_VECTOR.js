...
var ASM_CONSTS = {
  19630496: () => { if (window.__hideBootOverlay) { window.__hideBootOverlay(); } },  
 19630562: () => { try { var text = localStorage.getItem('nv_controls'); if (!text) { return 0; } var length = lengthBytesUTF8(text) + 1; var buffer = _malloc(length); stringToUTF8(text, buffer, length); return buffer; } catch (e) { return 0; } },  
 19630792: ($0) => { try { localStorage.setItem('nv_controls', UTF8ToString($0)); } catch (e) { } },  
 19630873: () => { try { var text = localStorage.getItem('nv_graphics_settings'); if (!text) { return 0; } var length = lengthBytesUTF8(text) + 1; var buffer = _malloc(length); stringToUTF8(text, buffer, length); return buffer; } catch (e) { return 0; } },  
 19631112: ($0) => { try { localStorage.setItem('nv_graphics_settings', UTF8ToString($0)); } catch (e) { } },  
